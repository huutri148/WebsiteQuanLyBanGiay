...
function Account(props) {
  ...
  const handleOnSubmit = (event) => {
    ...
    if (tabID === 0) {
      const loginParams = {
        TenDangNhap: user.loginEmail,
        MatKhau: user.loginPassword,
      };
      dispatch(DangNhap(loginParams));
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (isAuth) {
      ...
      if (typeof isSet === "undefined") dispatch(setNguoiDung(decode2.data));
      setLogin(true);
    }
  }, []);
